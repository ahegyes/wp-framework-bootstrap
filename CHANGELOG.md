...
Pending entries live in [`changelog/`](./changelog) — add via `composer packages:bootstrap:changelog:add` from the monorepo root. Aggregate into a release with `composer packages:bootstrap:changelog:write`.
...
- **Admin notice renderer** — renders an unmet-requirements `WP_Error` as a localized admin notice, scoped to the PHP and WordPress version codes it owns.
